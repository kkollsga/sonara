...
fn accuracy_bpm_120() {
    ...
    let is_harmonic =
        (r.bpm - 120.0).abs() < 15.0 || (r.bpm - 60.0).abs() < 15.0 || (r.bpm - 240.0).abs() < 15.0;
    assert!(
        is_harmonic,
        "BPM {} should be 120 or octave harmonic",
        r.bpm
    );
}
...
fn accuracy_bpm_90() {
    ...
    let r = sonara::analyze::analyze_signal(y.view(), 22050, &sonara::analyze::compact()).unwrap();
    let is_harmonic =
        (r.bpm - 90.0).abs() < 15.0 || (r.bpm - 180.0).abs() < 15.0 || (r.bpm - 45.0).abs() < 15.0;
...
fn accuracy_bpm_140() {
    ...
    let r = sonara::analyze::analyze_signal(y.view(), 22050, &sonara::analyze::compact()).unwrap();
    let is_harmonic =
        (r.bpm - 140.0).abs() < 15.0 || (r.bpm - 70.0).abs() < 15.0 || (r.bpm - 280.0).abs() < 15.0;
    assert!(
        is_harmonic,
        "BPM {} should be 140 or octave harmonic",
        r.bpm
    );
}
...
fn accuracy_centroid_440hz() {
    ...
    let r = sonara::analyze::analyze_signal(y.view(), 22050, &sonara::analyze::compact()).unwrap();
    assert!(
        (r.spectral_centroid_mean - 440.0).abs() < 50.0,
        "Centroid {} should be ~440 Hz",
        r.spectral_centroid_mean
    );
}
...
fn accuracy_centroid_1000hz() {
    ...
    let r = sonara::analyze::analyze_signal(y.view(), 22050, &sonara::analyze::compact()).unwrap();
    assert!(
        (r.spectral_centroid_mean - 1000.0).abs() < 100.0,
        "Centroid {} should be ~1000 Hz",
        r.spectral_centroid_mean
    );
}
...
fn accuracy_centroid_low_vs_high() {
    ...
    let r_low =
        sonara::analyze::analyze_signal(low.view(), 22050, &sonara::analyze::compact()).unwrap();
    let r_high =
        sonara::analyze::analyze_signal(high.view(), 22050, &sonara::analyze::compact()).unwrap();
    assert!(
        r_low.spectral_centroid_mean < r_high.spectral_centroid_mean,
        "200Hz centroid ({}) should be < 4000Hz centroid ({})",
        r_low.spectral_centroid_mean,
        r_high.spectral_centroid_mean
    );
}
...
fn accuracy_rms_sine() {
    ...
    let r = sonara::analyze::analyze_signal(y.view(), 22050, &sonara::analyze::compact()).unwrap();
    assert!(
        (r.rms_mean - 0.707).abs() < 0.1,
        "RMS {} should be ~0.707 for unit sine",
        r.rms_mean
    );
}
...
fn accuracy_rms_half_amplitude() {
    ...
    let r = sonara::analyze::analyze_signal(y.view(), 22050, &sonara::analyze::compact()).unwrap();
    assert!(
        (r.rms_mean - 0.354).abs() < 0.1,
        "RMS {} should be ~0.354 for 0.5 amplitude sine",
        r.rms_mean
    );
}
...
fn accuracy_dynamic_range_constant() {
    ...
    let r = sonara::analyze::analyze_signal(y.view(), 22050, &sonara::analyze::compact()).unwrap();
    assert!(
        r.dynamic_range_db < 5.0,
        "Dynamic range {} dB should be small for constant sine",
        r.dynamic_range_db
    );
}
...
fn accuracy_zcr_frequency_proportional() {
    ...
    let r_low =
        sonara::analyze::analyze_signal(low.view(), sr, &sonara::analyze::compact()).unwrap();
    let r_high =
        sonara::analyze::analyze_signal(high.view(), sr, &sonara::analyze::compact()).unwrap();
    assert!(
        r_high.zero_crossing_rate > r_low.zero_crossing_rate * 5.0,
        "ZCR for 4kHz ({}) should be much higher than 100Hz ({})",
        r_high.zero_crossing_rate,
        r_low.zero_crossing_rate
    );
}
...
fn accuracy_zcr_sine_expected_value() {
    ...
    assert!(
        (r.zero_crossing_rate - expected_zcr).abs() < 0.01,
        "ZCR {} should be ~{:.4}",
...
fn accuracy_onset_count_click_train() {
    ...
    let r = sonara::analyze::analyze_signal(y.view(), 22050, &sonara::analyze::compact()).unwrap();
    assert!(
        r.onset_frames.len() >= 3 && r.onset_frames.len() <= 12,
        "Expected ~8 onsets for 120 BPM/4s, got {}",
        r.onset_frames.len()
    );
}
...
fn accuracy_onset_density() {
    ...
    let r = sonara::analyze::analyze_signal(y.view(), 22050, &sonara::analyze::compact()).unwrap();
    assert!(
        r.onset_density > 0.5 && r.onset_density < 5.0,
        "Onset density {} should be roughly ~2 for 120 BPM",
        r.onset_density
    );
}
...
fn accuracy_silence_no_onsets() {
    ...
    let r = sonara::analyze::analyze_signal(y.view(), 22050, &sonara::analyze::compact()).unwrap();
    assert!(
        r.onset_frames.is_empty(),
        "Silence should have 0 onsets, got {}",
        r.onset_frames.len()
    );
}
...
fn accuracy_bandwidth_sine_vs_noise() {
    ...
    let r_sine =
        sonara::analyze::analyze_signal(sine_sig.view(), 22050, &sonara::analyze::playlist())
            .unwrap();
    let r_noise =
        sonara::analyze::analyze_signal(noise_sig.view(), 22050, &sonara::analyze::playlist())
            .unwrap();
    ...
    assert!(
        bw_sine < bw_noise,
        "Sine bandwidth ({}) should be < noise bandwidth ({})",
        bw_sine,
        bw_noise
    );
    ...
    assert!(
        bw_sine < 500.0,
        "Pure sine bandwidth {} should be very narrow",
        bw_sine
    );
}
...
fn accuracy_flatness_sine_vs_noise() {
    ...
    assert!(
        fl_sine < 0.1,
        "Sine flatness {} should be near 0 (tonal)",
        fl_sine
    );
    assert!(
        fl_noise > fl_sine,
...
fn accuracy_rolloff_low_vs_high() {
    ...
    let r_low =
        sonara::analyze::analyze_signal(low.view(), 22050, &sonara::analyze::playlist()).unwrap();
    let r_high =
        sonara::analyze::analyze_signal(high.view(), 22050, &sonara::analyze::playlist()).unwrap();

    let ro_low = r_low.spectral_rolloff_mean.unwrap();
    let ro_high = r_high.spectral_rolloff_mean.unwrap();
    assert!(
        ro_low < ro_high,
        "200Hz rolloff ({}) should be < 5000Hz rolloff ({})",
        ro_low,
        ro_high
    );
}
...
fn accuracy_mfcc_dimensions() {
    ...
    assert!(
        mfcc[0].abs() > 0.1,
        "MFCC[0] (log energy) {} should be non-zero for a signal",
        mfcc[0]
    );
}
...
fn accuracy_mfcc_different_timbres() {
    ...
    assert!(
        dist > 5.0,
        "MFCC distance between sine and noise ({}) should be large",
        dist
    );
}
...
fn accuracy_chroma_a440() {
    ...
    let a_bin = 9;
    let dist =
        ((max_bin as i32 - a_bin as i32).abs()).min(12 - (max_bin as i32 - a_bin as i32).abs());
    assert!(
        dist <= 1,
        "Strongest chroma bin {} should be near A (bin 9), got chroma: {:?}",
        max_bin,
        chroma
    );
}
...
fn accuracy_spectral_contrast_noise() {
    ...
    for (i, &v) in contrast.iter().enumerate() {
        assert!(
            v.is_finite(),
            "Contrast band {} should be finite, got {}",
            i,
            v
        );
    }
...
fn accuracy_centroid_fused_vs_standalone() {
    ...
    let standalone =
        sonara::feature::spectral::spectral_centroid(Some(y.view()), None, sr as Float, 2048, 512)
            .unwrap();
    ...
    let rel_diff =
        (fused.spectral_centroid_mean - standalone_mean).abs() / standalone_mean.max(1.0);
    assert!(
        rel_diff < 0.15,
        "Fused centroid ({}) vs standalone ({}) differ by {:.1}%",
        fused.spectral_centroid_mean,
        standalone_mean,
        rel_diff * 100.0
    );
}
...
fn accuracy_rms_fused_vs_standalone() {
    ...
    let standalone = sonara::feature::spectral::rms(Some(y.view()), None, 2048, 512).unwrap();
    let standalone_mean = standalone.row(0).iter().sum::<Float>() / standalone.ncols() as Float;
...
fn accuracy_perceptual_energy_loud_vs_quiet() {
    ...
    let r_loud =
        sonara::analyze::analyze_signal(loud.view(), sr, &sonara::analyze::playlist()).unwrap();
    let r_quiet =
        sonara::analyze::analyze_signal(quiet.view(), sr, &sonara::analyze::playlist()).unwrap();
    ...
    assert!(
        e_loud > 0.4,
        "Loud signal energy should be > 0.4, got {}",
        e_loud
    );
}
...
fn accuracy_perceptual_danceability_regular_beat() {
    ...
    let r_clicks =
        sonara::analyze::analyze_signal(clicks.view(), sr, &sonara::analyze::playlist()).unwrap();
    let r_noise =
        sonara::analyze::analyze_signal(noise.view(), sr, &sonara::analyze::playlist()).unwrap();
...
fn accuracy_perceptual_key_a440_extended() {
    ...
    assert!(
        key.starts_with("A") || key.starts_with("D"),
        "A440 should detect key A or D (where A is the 5th), got '{}'",
        key
    );
...
fn accuracy_perceptual_key_full_c_major() {
    ...
    let acceptable =
        key.contains("C") || key.contains("A") || key.contains("E") || key.contains("G");
    assert!(
        acceptable,
        "C major chord should detect a related key (C/Am/Em/G), got '{}'",
        key
    );
}
...
fn accuracy_perceptual_valence_major_vs_minor() {
    ...
    let r_major =
        sonara::analyze::analyze_signal(major.view(), sr, &sonara::analyze::playlist()).unwrap();
    let r_minor =
        sonara::analyze::analyze_signal(minor.view(), sr, &sonara::analyze::playlist()).unwrap();
...
fn accuracy_perceptual_acousticness_sine_vs_noise() {
    ...
    let r_sine =
        sonara::analyze::analyze_signal(y_sine.view(), 22050, &sonara::analyze::playlist())
            .unwrap();
    let r_noise =
        sonara::analyze::analyze_signal(y_noise.view(), 22050, &sonara::analyze::playlist())
            .unwrap();
